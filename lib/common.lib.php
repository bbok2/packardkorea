...
function get_paging($write_pages, $cur_page, $total_page, $url, $add="", $viewtype='1')
{
    ...
    if ($viewtype == '2') {
        ...
        $class_first = "";
        $class_last = "";
        $class_prev = "class='control prev'";
        $class_next = "class='control next'";
        $class_page = "";
        $class_cur_page = "class='active'";
    } else {
        ...
        // $showFirstLast = true;
        // $class_first = "class='pg_page pg_start'";
        // $class_last = "class='pg_page pg_end'";
        // $class_prev = "class='pg_page pg_prev'";
        // $class_next = "class='pg_page pg_next'";
        // $class_page = "class='pg_page'";
        // $class_cur_page = "class='pg_current'";
    }

    $str = '';
    if ($showFirstLast) { //$cur_page > 1 &&
        $str .= '<a href="' . $url . '1' . $add . '" '. $class_first .'>처음</a>' . PHP_EOL;
    }
    ...
    if ($start_page > 1) { $prevlink = $url.($start_page-1).$add; }
    else { $prevlink = "#"; }

    $str .= '<a href="'. $prevlink .'" '. $class_prev .'><span>이전</span></a>'.PHP_EOL;

    if ($total_page > 1) {
        for ($k=$start_page;$k<=$end_page;$k++) {
            if ($cur_page != $k)
                $str .= '<a href="'.$url.$k.$add.'" '. $class_page .'>'.$k.'</a>'.PHP_EOL;
            else {
                $str .= '<a href="'. $url.$k.$add .'" '. $class_cur_page .'>' . $k . '</a>' . PHP_EOL; //<strong class="pg_current"></strong>
            }
        }
    } else {
        $str .= '<a href="'. $url. '1' .$add .'" '. $class_cur_page .'>1</a>' . PHP_EOL; //<strong class="pg_current"></strong>
    }

    if ($total_page > $end_page) { $nextlink = $url.($end_page+1).$add; }
    else { $nextlink = "#"; }

    $str .= '<a href="'. $nextlink .'" '. $class_next .'><span>다음</span></a>'.PHP_EOL;

    if ($showFirstLast) { //$cur_page < $total_page &&
        $str .= '<a href="' . $url . $total_page . $add . '" '. $class_last .'>맨끝</a>' . PHP_EOL;
    }

    if ($str) {
        if ($viewtype == "2") {
            return '<div class="pagenavi_w pagenavi">' . $str . '</div>';
        } else {
            return '<div class="pagenavi_w pagenavi">' . $str . '</div>';
...
function print_rr($var, $end=false) {
    print_r2($var);
    if ($end) { exit; }
}
...
function sql_result($sql_query_result, $jsonfield=null) {
    $result_array = array();
    while ($row = sql_fetch_array($sql_query_result)) {
        if ($jsonfield != null && is_array($jsonfield) && count($jsonfield)>0) {
            foreach ($jsonfield as $jrow) {
                $row[$jrow] = json_decode($row[$jrow], true);
            }
        }
        $result_array[] = $row;
    }
...
function get_token($tp='')
{
    $tkname = 'ss_token';
    if ($tp != '') { $tkname .= "_" . $tp; }
    $token = md5(uniqid(rand(), true));
    set_session($tkname, $token);

    return $token;
}

function valid_token($val, $tp='') {
    $tkname = 'ss_token';
    if ($tp != '') { $tkname .= "_" . $tp; }
    if ($val != "" && $val == get_session($tkname)) {
        set_session($tkname, '');
        return true;
    }
    return false;
...
function get_uniqid($qx=6, $sep='', $rtntype='o')
{
    global $g5;
    /*  데이터가 너무 많이 쌓이는 문제 발생으로 인해 주석처리 */
    ...
    if ($rtntype == 'o') {
        ...
        list($usec, $sec) = explode(' ', microtime());
        $usec = str_pad(((float)$usec * 1000000), 6, '0', STR_PAD_LEFT);    //6자리
        $usec = substr($usec . mt_rand(), 0, $qx);
        $key = date('ymdHi', time()) . $sep. date('s', time()) . str_pad($usec, $qx, "0", STR_PAD_LEFT);
    }
    else if ($rtntype == 'a') { //$qx 무시됨
        $key = uniqid();
    }
    else if ($rtntype == 'A') { //$qx 무시됨
        ...
        $key = uniqid();
        if ($rtntype == 'D') { $key = strtoupper($key); }
        if ($qx < 8) { $qx = 8; }

        $key = substr($key, -1*$qx);
        $key = date('ymd', time()) . $sep. $key;
    }
...
function conv_date_format($format, $date, $add='')
{
    ...
    if (strpos($format, 'w') !== false) { $format = str_replace('w', '@|@', $format); }
    else if (strpos($format, 'W') !== false) { $format = str_replace('W', '@|@|', $format); }

    $yoil = array("일","월","화","수","목","금","토");
    $dt = date($format, $timestamp);

    if (strpos($format, '@|@') !== false) {
        $str_yoil = $yoil[date('w', strtotime($date))];
        if (strpos($format, '@|@|') !== false) {
            $dt = str_replace('@|@|', $str_yoil . "요일", $dt);
        }
        else {
            $dt = str_replace('@|@', $str_yoil, $dt);
        }
    }
...
//파일 업로드
//옵션 : regAllowExt='', regDisAllowExt='', imgsize=array(width,height),  리사이징?
function upload_uniqfile($dir, &$files, $options=array()) {
    $rtnfile = $listfile = array();
    ...
    foreach ($listfile as $fk=>$file) {
        $upload_ext = true;

        if (isset($options['regAllowExt']) && !preg_match($options['regAllowExt'], $file['name'])) { $upload_ext = false; }
        if ($upload_ext && isset($options['regDisAllowExt']) && preg_match($options['regDisAllowExt'], $file['name'])) { $upload_ext = false; }

        if (is_uploaded_file($file['tmp_name']) && $upload_ext) {
            $orgname = $file['name'];
            $realsavedname = '';

            if ($upload_ext) {
                $finfo = getfile_exp_name($file['name']);
                $dest_path = $dir . get_uniqid(10) . '.' . $finfo['ext'];
                $dest_path = getfile_uniq_name($dest_path);
                $full_path = $dest_path['dir'] . '/' . $dest_path['name'];
                move_uploaded_file($file['tmp_name'], $full_path);
                @chmod($full_path, G5_FILE_PERMISSION);

                $orgname = $finfo['name'] . '.' . $finfo['ext'];
                $realsavedname = $dest_path['name'];

                if (isset($options['imgsize'])) { //이미지 검사 옵션이면
                    if (file_exists($full_path)) {
                        $imgsize = getimagesize($full_path);
                        if ($imgsize[0] > $options['imgsize']['width'] || $imgsize[1] > $options['imgsize']['height']) {
                            @unlink($full_path);
                            $file['error'] = 7;
                        }
                    }
            ...
            $filearr = array('orgname'=>$orgname, 'realname'=>$realsavedname, 'type'=>$file['type'], 'size'=>$file['size'], 'error'=>$file['error']);
            if (isset($options['keyRetain']) && $options['keyRetain'] == 'Y') {
                $rtnfile[$fk] = $filearr;
            }
            else {
                $rtnfile[] = $filearr;
...
//파일의 이름과 확장자 분리
function getfile_exp_name($fname) {
    $path = str_replace("\\", "/", $fname);
    $dir = '';

    if (strpos($fname, "/") !== false) {
        $path_array = explode("/", $path);
        $fname = $path_array[count($path_array)-1];
        unset($path_array[count($path_array)-1]);
        $dir = implode("/", $path_array);
    }
    ...
    return array('dir'=>$dir, 'name'=>$file_name, 'ext'=>$file_ext);
}

function getfile_uniq_name($path) {
    $fileinfo = getfile_exp_name($path);
    $filename = $fileinfo['name'] . '.' . $fileinfo['ext'];

    $num = 1;
    while(is_file($fileinfo['dir'] . "/" . $filename)) { // 중복파일 있을때 중복파일없을때까지 $num 1씩 올려서 중복파일체크
        $filename = $fileinfo['name'] ."_" . $num. "." . $fileinfo['ext'];
        $num++;
    }

    return array('dir'=>$fileinfo['dir'], 'name'=>$filename);
}

function param_make($params, $name=null) {
    $ret = "";
    foreach($params as $key=>$val) {
        if(is_array($val)) {
            if($name==null)
                $ret .= param_make($val, $key);
            else
                $ret .= param_make($val, $name."[$key]");
        } else {
...
//get 방식의 주소에서 불필요항목 삭제
function query_except($exception='', $querystring='')
{
    if ($querystring == '') {
    ...
    if($exception) {
        ...
        $querystring = param_make($newqs);
        $querystring = str_replace('&&', '&', str_replace("+", "%2B", $querystring));
...
function ajax_json($cd='0000', $msg='SUCCESS', $_etcMergeArray=array()) {
    $rtn = array("resCode"=>$cd, "resMsg"=>$msg);
    if (count($_etcMergeArray) > 0) {
        $rtn = array_merge($rtn, $_etcMergeArray);
    }
